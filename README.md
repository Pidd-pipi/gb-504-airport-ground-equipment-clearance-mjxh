...
- `/clearance`：形成 `cleared / restricted / revoked` 决定；完全放行同时要求全部关联设备可用，限制放行必须填写运行条件，紧急撤销不受未完成检查阻断。撤销后可由安全放行员发起复议：关联设备全部恢复且检查无待办或失败项时，撤销决定退回 `pending`、周转退回 `checking`，面板标注来源撤销并在审计保留原撤销记录。
...
| POST | `/clearance/:id/reconsider` | 撤销决定复议退回待决定 | 管理角色 |
